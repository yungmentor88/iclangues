...
export function HomeContent({ cmsFaqs }: { cmsFaqs: { question: Record<string,string>; answer: Record<string,string> }[] | null }) {
  const { t, lang } = useI18n();

  /**
   * FAQs from the CMS, resolved for the current language so they follow the
   * language switcher rather than freezing at English. Falls back to the
   * hard-coded list when the CMS is unreachable or has no published rows.
   */
  const faqItems =
    cmsFaqs && cmsFaqs.length > 0
      ? cmsFaqs.map((f) => ({
          q: f.question?.[lang]?.trim() || f.question?.en || "",
          a: f.answer?.[lang]?.trim() || f.answer?.en || "",
        }))
      : FAQ;
  ...
  return (
    <main>
      ...
      <section className="container grid gap-10 py-20 sm:py-28 lg:grid-cols-[0.8fr_1.2fr]">
        ...
        <Reveal delay={0.1}><Faq items={faqItems} /></Reveal>
      </section>
